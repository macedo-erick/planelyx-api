...
import br.com.fintrackapi.repository.InvoiceRepository;
...
public class InvoiceService {
    ...
    public BillingPeriod resolveBillingPeriod(CreditCard card, LocalDate transactionDate) {
        ...
        YearMonth dueMonth =
                card.getDueDay() <= card.getClosingDay() ? periodEndMonth.plusMonths(1) : periodEndMonth;
    ...
    public List<Invoice> findAll(UUID ownerId, UUID creditCardId, InvoiceStatus status) {
        List<Invoice> invoices = creditCardId != null
        ...
        return invoices.stream().filter(invoice -> derivedStatus(invoice) == status).toList();
    }
